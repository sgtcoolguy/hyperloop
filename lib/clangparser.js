		...
		stream.on('data',function(line){
			...
			if (start > 0) {
				if (current.start === start) {
					DEBUG_TREE && log.debug(rpad(25, rpad(4,index-1), '|', rpad(4,start), '|', rpad(10,'sibling'), '|', rpad(3,current.index)), '|', rpad(20,node.token), line.substring(0,80));
					current.parent.add(node);
				}
				else {
					if (current.start < start) {
						DEBUG_TREE && log.debug(rpad(25, rpad(4,index-1), '|', rpad(4,start), '|', rpad(10,'child'), '|', rpad(3,current.index)), '|', rpad(20,node.token), line.substring(0,80));
						current.add(node);
					}
					else {
						...
						while(p) {
							if (p.start === start) {
								DEBUG_TREE && log.debug(rpad(25, rpad(4,index-1), '|', rpad(4,start), '|', rpad(10,'sibling'), '|', rpad(3,p.index)), '|', rpad(20,node.token), line.substring(0,80));
								p.parent.add(node);
								break;
							}
							p = p.parent;
				...
				DEBUG_TREE && log.debug(rpad(25, rpad(4,index-1), '|', rpad(4,start), '|', rpad(10,'root'), '|', rpad(3,-1)), '|', rpad(20,node.token), line.substring(0,80));
				root = node;
			}
			current = node;
