...
function downloadResourceIfNecessary(name, version, url, checksum, dir, callback) {
	...
	var verFn = path.join(dir,name+'-version.txt'),
		...
		resultExists = name !== 'ios-sim' || fs.existsSync(path.join(zipdir, name));
				log.info('downloadResourceIfNecessary verFn ' + verFn);
				log.info('downloadResourceIfNecessary zipdir ' + zipdir);
				log.info('downloadResourceIfNecessary localVersion ' + localVersion);
				log.info('downloadResourceIfNecessary zf ' + zf);

	if (version !== localVersion || !resultExists) {
				log.info('downloadResourceIfNecessary !localVersion or !resultExists');
		...
		if (!fs.existsSync(zipdir)) {
				log.info('downloadResourceIfNecessary mkdirSyncRecursive zipdir ' + zipdir);
		...
		req.on('response', function(res) {
			...
			res.on('end', function() {
				...
				log.info('downloadResourceIfNecessary checkChecksum ' + checkChecksum);
