...
commands['compile'] = new Command('compile', 'compile source files', function(options, args, callback) {
	...
	if (isDir && !fs.existsSync(path.join(src, 'app.hjs')) &&
		...
		log.fatal('No app.hjs or app.js file found in', src.magenta);
	}

	var files = isDir ? filelisting(src, /\.h?js$/, undefined, options.dest) : [src];
	if (files.length === 0) {
	...
	codegen.generate(asts, generateASTs, function codegenGenerateCallback(err) {
			log.info('codegen.generate entered');
		
		finishedCommand('compile');
			log.info('codegen.generate passed finishedCommand ');
		
		err && log.fatal(err);
		if (callback) {
			log.info('codegen.generate invoking callback ' + callback);
			
			callback();
			log.info('codegen.generate done invoking callback ' + callback);
			
		}
		else {
			log.info('codegen.generate no callback ');
			
			process.exit(0);
		}
			log.info('codegen.generate end ');
