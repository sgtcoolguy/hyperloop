...
var MIN_IOS_VERSION = '7.0',
	TMP_DIR = process.env.TMPDIR || util.writableHomeDirectory(),
...
function loadMetabase(file, opts, callback) {
	...
	// if not ti.current, we need to enforce min hyperloop ios, otherwise, we don't enforce
	if (!opts.options.ticurrent) {
	...
	// generate a new metabase
		...
		execClang(file, opts, function(err, fn) {
			...
			clangparser.parseFile(buildlib, fn, function(err, ast) {
	...
	buildlib.getXcodeSettings(function(err, settings) {
		...
		var stdout = '',
			...
			headers = frameworks.map(function(fw) {
				...
				if (opts.frameworks_to_exclude.indexOf(fw.replace(/\.framework/,''))!==-1) {
					return;
				}
		...
		var args = [].concat(
				util.format('-Xclang -ast-dump -miphoneos-version-min=%s -isysroot %s -arch i386 -x objective-c --analyze -fno-color-diagnostics', opts.minVersion, settings.simSDKPath).split(/\s+/),
				file
			);
