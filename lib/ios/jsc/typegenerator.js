 ...
 */
function createState (metadata, version, referenceTable, classprefix, ticurrent) {
	return {
		metadata: metadata,
		version: version,
		dependencies: [],
...
function resolveType (state, type) {
	...
	if (!type.trim && typeof(type)=='object') return type;
	type = type.trim();
	...
	var simpleType = convertToSimpleType(type),
		...
		is_object = !!(classObject || protoObject) || /^(SEL|id|Class)$/.test(simpleType),
		...
		is_struct = !is_object && (typedObject && ((typedObject.type && /(definition|struct)/.test(typedObject.type)) || (typedObject.subtype && typedObject.subtype.indexOf('struct ')!==-1))) || false,
	...
	if (type=='id *' || type=='const id *') {
		is_pointer = false;
		simpleType = name = type = realtype = mangled = 'id';
		typedObject = null;
		classObject = {};
		is_object = true;
		is_struct = false;
	...
	if (is_array) {
		...
		args.filter(function(e){return e}).forEach(function(n){
			if (n==='...') {
			...
			if (!a) {
				throw new Error("couldn't find type: "+n);
			}
...
function makeGetProperty (state, typeobj, property, propertyName, resultName, instanceName) {
	...
	if (propobj.simpleType=='id' && property.type!='id') {
		// its a pointer to a id, let's just ignore
		code.push('// return a pointer to an id is not supported');
...
function makeSetProperty (state, typeobj, property, propertyName, instanceName, argumentsName, argumentsLengthName) {
	...
	if (typeobj.metatype==='struct') {
		operator = '->';
		if (propobj.simpleType=='id' && property.type!='id') {
			return '// setting a id * is not supported\nreturn false;';
		}
		varassign = makeVarAssignmentFromJSValueRef(state, typeobj, propobj, propertyName+'$'+0, 'value', cleanup);
	}
	else {
...
function makeMethod (state, typeobj, methodName, resultName, instanceName, argumentsName, argumentsLengthName, returnFilterFunc, useSuperCheck) {
	...
	_.compact(methods).forEach(function(method) {
		...
		method.returnType = resolveType(state, method.returnType||method.returnSubtype);
		method.returnSubtype = resolveType(state, method.returnSubtype||method.returnType);
		...
		if (has_args) {
			...
			if (method.formatter) {
				...
				if (method.args[0].simpleType=='id' && method.args[0].argument.type!='id') {
				...
				for (var c=1;c<method.args.length;c++) {
					...
					if (method.args[c].simpleType=='id' && method.args[c].argument.type!='id') {
		...
		if (!is_void && !method.requiresSentinel) {
			if (method.formatter) {
				...
				if (!useSuperCheck) {
					if (/(^init$)|(^init[A-Z])|(new|alloc)$|(([cC]opy|[aA]lloc)WithZone)/.test(method.name) ||(method.returnType.type==='id' && method.returnSubtype==='instancetype')) {
...
function makeBlock(state,object) {
	...
	code.push('\tif ([[NSThread currentThread] isEqual:thread])');
	code.push('\t{');
	code.push('\t\tBlockFn$();');
	if (!blockReturn.is_void) {
	...
	code.push('\t{');
	if (state.ticurrent) {
		code.push('\t\tHyperloopPerformBlockOnKrollThread(thread,BlockFn$,'+(blockReturn.is_void?'NO':'YES')+');');
	}
	else {
...
function generateBlock(state, object) {
	...
	return ejs.render(block_to_native_template,{
		...
		indentify: indentify,
		classprefix: state.classprefix,
		thread: state.ticurrent ? 'currentThread' : 'mainThread'
	});
}
...
function generateCustomClass(state,name,classdef,srcdir,srcs,version) {
	...
	copymethods.forEach(function(method) {
		...
		code.push('if (fn$==NULL)');
		code.push('{');
		code.push('\tNSLog(@"[ERROR] cannot find function named %s", "'+method.action_name+'");');
		code.push('\treturn '+(returnType.is_void?';':'0;'));
		code.push('}');
	...
	Object.keys(typeobj.class_methods).sort().forEach(function(m) {
	    var code = makeMethod(state, typeobj, typeobj.class_methods[m], 'result', 'instance', 'arguments', 'argumentCount', function(resultname,instancename){
	    	var mt = typeobj.class_methods[m][0].returnSubtype;
	    	if (mt && (mt.simpleType==='instancetype' || mt.simpleType==='id')) {
		    	var code = [];
