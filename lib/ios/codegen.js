...
iOSCodegen.prototype.generate = function(asts,generateASTCallback,callback) {
	...
	if (options.ticurrent) {
		minversion = options['min-version'] = '6.0';
		...
		frameworks_to_exclude.push('SpriteKit');
	}
	...
	buildlib.getSystemFrameworks(function(err,frameworks){
		...
		frameworks.forEach(function(f){
			...
			if (system_frameworks.indexOf(name)===-1 && frameworks_to_exclude.indexOf(name)===-1) {
		...
		buildlib.getXcodeSettings(function(err,settings){
						...
						if (gen.cached) {
							...
							tasks.push(function(callback){
								...
								zlib.gzip(sourcecode,function(err,buf){
									...
									gen.prefix = prefix;
									try {
										if (options.debug && /(_source_map|_sm)$/.test(gen.name)) {
											gen.sourcecode='';
										}
										else {
											gen.sourcecode = options.debug && Uglify.parse(sourcecode,{}).print_to_string({beautify:true});
										}
									}
									catch (E) {
										gen.sourcecode = '';
			...
			metabase.loadMetabase(
				headerfile,
				{
					...
					options: options,
					frameworks_to_exclude: frameworks_to_exclude
				},
				function(err, result) {
					if (err) { return callback(err); }
