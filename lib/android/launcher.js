...
AndroidLauncher.prototype.launch = function(options, args, callback) {
	...
	programs.msbuild('"' + paths.solutionFile + '" /p:Platform="Android32"', function didBuild(err) {
		if (err) {
			...
			bar && setTimeout(findAppX, 50) || findAppX();
		}
	...
	function askForSideloading() {
		...
		log.error('\t1. Click to expand ' + 'Computer Configuration'.bold + ', ' + 'Administrative Templates'.bold + ', ' + 'Androiddows Components'.bold + ', and then ' + 'App Package Deployment'.bold + '.');
		log.error('\t2. Double-click the ' + 'Allow development of Androiddows Store apps without installing a developer license'.bold + ' setting.');
		log.error('\t3. In the Allow development of Androiddows Store apps without installing a developer license window, click ' + 'Enabled'.bold + ' and then click ' + 'OK'.bold + '.');
