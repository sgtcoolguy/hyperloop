...
function mergeTemplateFiles(options, args, paths, callback) {
	...
	for (var symbol in options.imports) {
		hl_app_code += 'var ' + symbol + '=' + options.imports[symbol] + ';';
	}

	hl_app_code += fs.readFileSync(path.join(paths.destDir, 'src', 'hl_app.js'), 'utf8').replace(/\"/g, '\\"');
