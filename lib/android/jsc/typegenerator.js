...
var fs = require('fs'),
	...
	log = require('../../log'),
	util = require('../../util'),
	primitiveTypeRegex = /^(short|float|double|long|boolean|int|char)/i,
...
function isSymbolReferenced(state, className, name, type) {
	...
	return state.referenceTable[name];
}

/**
 * create a state object that we can use during compilation and that can be passed along to functions
 */
function createState(generate, metadata, version, referenceTable) {
	var state = {
		generate: generate,
		generated: {},
		generics: generate.generics,
		metadata: metadata,
		version: version,
		handlerBindings: {},
		dependencies: [],
...
function available(obj, version) {
	...
	if (availability && (availability.message === 'Unavailable' || availability.platform !== 'android')) {
		return false;
	}
	if (obj.unavailable) {
		return false;
...
function updateIsInstancePropertyIfEmpty(method) {
	if (method.instance === undefined || method.instance === null) {
		method.instance = (method.attributes && method.attributes.indexOf('static') == -1);
	}
}


function resolveObject(state, result) {
	log.debug('typegenerator.resolveObject(' + result.name + ')');
	var simpleType = result.simpleType,
	...
	var methodsProcessed = !!result.methods;
	...
	if (result.object.superClass) {
		pushInclude(state, includes, result.object.superClass);
	}
	...
	result.object.methods && Object.keys(result.object.methods).forEach(function(ii) {
		...
		var isDelegatesInvoke = result.is_delegate && name === 'Invoke';
		if (!isSymbolReferenced(state, simpleType, name, 'methods') && !isDelegatesInvoke) {
			return;
		}
		var copymethods = result.methods[name] || [],
			skip = false;

		var isPrivate = method.attributes && method.attributes.indexOf('public') === -1;
		...
		if (method.args && method.args.length) {
			method.args.forEach(function(a) {
				...
				rt && rt.className && !rt.is_generic && pushInclude(state, includes, rt.className);
				var rtc = _.clone(rt);
				rtc.argument = a;
				args.push(rtc);
			});
		...
		if (isDelegatesInvoke) {
			result.delegate_args = args;
		}

		if (!isPrivate && name !== '<init>') {
			updateIsInstancePropertyIfEmpty(method);
			if (method.instance) {
				...
				log.debug('____instance method: ' + simpleType + '.' + name);
			}
			else {
				...
				log.debug('____class method: ' + simpleType + '.' + name);
			}
		}

		if (!methodsProcessed && copymethods) {
			copymethods.push(m);
	...
	result.object.fields && Object.keys(result.object.fields).forEach(function(name) {
		...
		if (!p) {
			delete result.object.fields[name];
		}
		p && p.className && pushInclude(state, includes, p.className);	
	});
	...
	result.object.properties && Object.keys(result.object.properties).forEach(function(name) {
		var property = result.object.properties[name],
			...
			availability = property.availability || { platform: 'android' };

		if (availability.message === 'Unavailable' || availability.platform !== 'android') {
			return;
		}
		...
		p.className && pushInclude(state, includes, p.className);
	...
	while (sc) {
		...
		if (sc.superClass.className == 'java.lang.Object' && sc.className == 'java.lang.Object') {
			break;
		}
		sc = sc.superClass;
	}
		...
		if (extendsClass) {
			[
				...
				'readonly_properties', 'instance_methods', 'class_methods', 'properties'
					...
					if (key === 'methods' && extendsClass.methods && extendsClass.methods._ctor) {
						// Don't copy in constructors from super classes.
						delete extendsClass.methods._ctor;
					}
					_.defaults(result[key], extendsClass[key]);
				});
			state.includes[result.type] = state.includes[result.type].concat(state.includes[extendsClass.type]);
		}
...
function resolveType(state, type) {
	...
	// turn the type into something that we can lookup -- for example, strip all the extra typing information that isn't
	// important to convert this type object into a type we can deal with
	var simpleType = convertToSimpleType(type),
		metadata = state.metadata,
		matchAgainst = '\\.' + (is_generic ? type.split('<')[0] + '(`\\d)?<' : type + '$'),
	...
	if (matchingClassNames.length > 1) {
		matchingClassNames = disambiguate(type, state.generate.compiler, matchingClassNames, originalType);
	}
	var includes = [],
		matchingClassName = matchingClassNames && matchingClassNames.length && matchingClassNames[0],
		fullInstanceName = matchingClassName,
		...
		is_primitive = primitiveTypeRegex.test(simpleType),
		...
		realtype = type,
		javaPackage = fullInstanceName && fullInstanceName.substring(0, fullInstanceName.lastIndexOf('.')),
		generatedPackage = fullInstanceName && 'generated.' + javaPackage;

	// only allow through generics that use specific types
	if (is_generic) {
		if (type.indexOf('!') >= 0) {
			//log.trace('Skipping ' + type + ' because it is a generic without specific types.');
			return null;
		}
		if (type.split('<').length !== type.split('>').length) {
			log.trace('Skipping ' + type + ' because the generic looks invalid (< and > are imbalanced).');
			return null;
		}
		if (BLACKLIST_CLASSES.filter(function (f) { return simpleType.indexOf(f) >= 0; }).length > 0) {
			return null;
		}
	}
	...
	if (!is_void && !is_function_pointer && !typeObject && !is_primitive && !is_block && !is_void_pointer) {
		if (type in (state.customclasses || {})) {
			...
			return {
				...
				is_interface: true,
				is_object: true,
				instanceName: trimJavaPackageFromType(name),
				fullInstanceName: '',
	...
	if (is_generic) {
		generic_types = breakApartGenericParams(type)
			.map(function(type) {
				return type.indexOf('Object') >= 0 || resolveType(state, type);
			})
		...
		mangled = type.split('`')[0] + '$' + (generic_types.map(function(a) {
			a.className && pushInclude(state, includes, a.className);
			return a.mangledName;
		}).join('_')) + '$';
		fullInstanceName = realtype = name = simpleType = matchingClassName.split('`')[0] + '<' + (generic_types.map(function(a) {
			...
			return a.fullInstanceName + (a.is_struct ? '' : '^');
		}).join(', ')) + '>';
	}
			...
			if (typeObject.fields) {
				...
				Object.keys(typeObject.fields).forEach(function(name) {
					...
					if (p && p.className && p.className.indexOf('.') > 0) {
						pushInclude(state, includes, p.className);
					}
				});
			}
		}
...
function makeVarAssignmentFromJSValueRef(state, typeobj, arg, name, argname, cleanup) {
	...
	var type = arg.type,
		...
		generatedClassDefinition = argobj.generatedPackage + '.' + argobj.instanceName,
		fnname;

	if (argobj.is_primitive) {
		fnname = 'Hyperloop.' + mangledName + 'FromJSValueRef';
	} else {
		fnname = generatedClassDefinition + '.valueOf';
	}

	varassign = argobj.type.trim();
	if (argobj.className && includes.indexOf(argobj.className) === -1) {
		if (argobj.is_generic) {
			pushInclude(state, includes, argobj.fullInstanceName);
		}
		else {
			pushInclude(state, includes, argobj.className);
		}
	}

	code.push(type + ' ' + name + ' = ' + fnname + '(ctx,' + argname + ',exception);');
...
function convertToJSValueRef(state, typeobj, mangledName, arg, resultName) {
	var is_primitive = primitiveTypeRegex.test(mangledName);
	var code, fnname;

	if (is_primitive) {
		fnname = 'Hyperloop.' + mangledName + 'ToJSValueRef';
	} else {
		var generatedClassDefinition = typeobj.generatedPackage + '.' + typeobj.instanceName;
		fnname = generatedClassDefinition + '.toJSValueRef';
	}
	...
	if (arg.is_primitive && !arg.is_pointer_to_pointer) {
		if (mangledName.indexOf('Array') >= 0) {
			code = 'JSValueRef ' + resultName + ' = ' + fnname + '(ctx, ' + resultName + '$->begin(), ' + resultName + '$->Length);';
		}
		else {
...
function makeExceptionHandler(code, returnValue) {
	var lineCount = code.length + 9; // back toward the top of the function
	...
	code.push('\t\t\t\t\tandroid.util.Log.d("JavaScriptCore", "Failed to invoke Java method", e);');
	code.push('\t\t\t\t\tHyperloop.RaiseNativeToJSException(ctx, e, ' + lineCount + ', exception);');
...
function makeMethod(state, typeobj, methodName, resultName, instanceName, argumentsName, argumentsLengthName, returnFilterFunc) {
	...
	if (methods[0] && typeof(methodName) === 'object') {
	...
	_.compact(methods).forEach(function(method) {
		var returnType = resolveType(state, method.returnType.type),
		...
		var target = convertToVarName(typeobj.mangledName);
		...
		if (has_args) {
			for (var c = 0; c < method.args.length; c++) {
				var arg = method.args[c],
					...
					va = makeVarAssignmentFromJSValueRef(state, typeobj, varg, 'arg$' + c, argumentsName + '.get(ctx, ' + c + ')', cleanup);

				// If the varg type is an array, but we're overriding IVector, wrap the array in an Array.
				var isIVector = method.overrides && method.overrides.type && method.overrides.type.indexOf('IVector') >= 0,
					isToArray = method.name.slice(-5) === 'Array' && arg.type.slice(-1) === '^' || (arg.mangledName && arg.mangledName.slice(-5) === 'Array');
				if (varg.type.indexOf('[]') > 0 && (isIVector || isToArray)) {
					...
					va.code += '\n\tauto ' + va.varname + '$Ref = ref new Array<' + varg.type.split('[]')[0] + '>(' + va.varname + ', ' + va.varname + '$Length);';
					va.varname += '$Ref';
				}
		...
		if (!is_void && !method.requiresSentinel) {
			if (method.formatter) {
				fnbody = 'Object ' + resultName + '$ = (' + returnType.type + ') ' + resultName + '$$;';
			}
			else {
				if (returnType.type === 'id' && method.returnSubtype === 'instancetype') {
					...
					fnbody = returnType.type + ' ' + resultName + '$ = ' + fnbody;
				}
			...
			if (!returnbody) {
				...
				if (returnType !== 'void' && returnType.metatype !== 'primitive' && returnType.className) {
					pushInclude(state, includes, returnType.fullInstanceName);
				}
				var rtt = returnType.type ? returnType : resolveType(state, returnType);
				returnbody = convertToJSValueRef(state, typeobj, returnType.mangledName, rtt, resultName);
			}
		}
		else {
			if (method.formatter) {
		...
		if (has_args) {
			for (var d = 0; d < method.args.length; d++) {
				var arg2 = method.args[d],
					varg2 = arg2.argument || arg2;
				if (varg2 && varg2.inout === 'out' && varg2.type.indexOf('valuetype ') === -1) {
					if (varg2.type.indexOf('class ') === -1) {
						log.trace('Skipping method out handling for ' + method.name + ' because out arg of type ' + varg2.type + ' is not yet supported.');
					}
					else {
						var varName = varg2.name + '$' + d,
							varPrefix = indent + 'auto ' + varName;
						code.push(varPrefix + '$O = JSValueToObject(ctx, arguments[' + d + '], exception);');
						code.push(varPrefix + '$PO = reinterpret_cast<JSPrivateObject*>(JSObjectGetPrivate(' + varName + '$O));');
						var storeInto = varg2.type.indexOf('class ') !== -1 ? 'object' : 'buffer';
						// TODO: If 'buffer', we need to actually resize and store in to the buffer.
						code.push(indent + varName + '$PO->' + storeInto + ' = ' + varName + ';');
					}
				}
			}
		}
	...
	if (methods.length > 1) {
		code.push('}');
		code.push('');
		code.push('return jsc.JSValueMakeUndefined(ctx);');
	}

	makeExceptionHandler(code, 'jsc.JSValueMakeUndefined(ctx)');
...
function generateInterface(state, name, obj) {
	log.debug('generateInterface(' + name + ')');
	!obj && (obj = resolveType(state, name));
	name = obj.mangledName;

	var includes = _.uniq(state.includes[obj.type] || []).sort(),
		constructors = obj.methods && obj.methods._ctor && obj.methods._ctor.filter(function(method) {
			return method.attributes.indexOf('public') >= 0 || state.handlerBindings[name];
		}),
...
function generateCast(state, name, symbol) {
	log.debug('generateCast(' + name + ')');
	var obj = resolveType(state, symbol.argType) || state.generics[symbol.argType],
		templateArgs = {
			entry: obj.object || {},
...
function generateGeneric(state, name, symbol) {
	log.debug('generateGeneric(' + name + ')');
	var templateArgs = {
		args: symbol.args,
...
function findSuperClassMethod(state, name, typeobj) {
	...
	while (sc) {
	    ...
	    if (sc.superClass && sc.className === 'java.lang.Object' && sc.superClass.className === 'java.lang.Object') {
	    	break;
	    }
		sc = sc.superClass;
	}
...
function generateCustomClass(state, name, classdef, srcdir, srcs, version) {
	log.debug('generateCustomClass ' + name);
	var methods = [],
		includes = [],
		instance_methods = {},
	...
	if (extendsType) {
		pushInclude(state, includes, extendsType.className);
	}
	...
	classdef.methods.forEach(function(method) {
		...
		if (method.arguments) {
			for (var c = 0; c < method.arguments.length; c++) {
				...
				type && type.className && arg.type != 'object' && pushInclude(state, includes, type.className);
				arg.typeObject = type;
				args.push(arg);
			}
		}
	...
	copymethods.forEach(function(method) {
		...
		if (!returnType.is_void) {
			...
			code.push('if (exception$ != NULL) throw exception$;');
	...
	Object.keys(typeobj.class_methods).sort().forEach(function(m) {
		...
		class_methods[m] = makeMethod(state, typeobj, classMethod, 'result', 'instance', 'arguments', 'argumentCount');
	});
	...
	var header = util.renderTemplate('templates/custom_class_header.ejs', {
		...
		implementation = util.renderTemplate('templates/custom_class.ejs', {
			...
			name: typeobj.name,
			varname: convertToVarName(typeobj.mangledName),
			instanceName: typeobj.instanceName,
			interfaces: classdef.interfaces,
			extendsType: extendsType,
			_includes: _includes,
...
function trimJavaPackageFromType(type) {
	var originalType = type.trim()
...
function disambiguate(type, compiler, matches, originalType) {
	var usingNamespace = compiler.using_namespaces || [],
		match;
	...
	for (var j = 0, jL = matches.length; j < jL; j++) {
		match = matches[j];
		for (var k = 0, kL = usingNamespace.length; k < kL; k++) {
			if (usingNamespace[k] === match.split('.').slice(0, -1).join('::')) {
				return [ match ];
			}
		}
	}
...
/**
 * Breaks a generic type's string in to an array of its parameters.
 * @param type Such as "IKeyValue`2<String^,IMap<String^,String^>^>"
 * @returns {Array} Such as [ "String^", "IMap<String^,String^>^" ]
 */
function breakApartGenericParams(type) {
	// Trim down to only what's between the outermost < and >.
	var params = type.slice(type.indexOf('<') + 1, -1);
	// Quickly handle simple generics.
	if (params.indexOf('<') === -1) {
		return params.split(',');
	}
	// Nested generics require a bit more iteration to break them down.
	var depth = 0,
		matches = [],
		matchFrom = 0;
	...
	for (var i = 0, iL = params.length; i < iL; i++) {
		var c = params[i],
			isLast = i === iL - 1;
		// If we find an opening <, increase our "depth".
		if (c === '<') {
			depth++;
		}
		// Decrease if we find a closing >.
		else if (c === '>') {
			depth--;
		}
		// If we find a comma, and we're not nested inside <>, we found a match!
		if (c === ',' && depth === 0) {
			matches.push(params.substr(matchFrom, i));
			matchFrom = i + 1;
		}
		// If we reach the end of the string, and we're not at 0-depth, this is a bad string.
		if (isLast && depth !== 0) {
			log.trace('skipping ' + type.bold + ' because it has imbalanced generics');
			return [];
		}
		// Otherwise, we can push in the last part of our string.
		else if (isLast && depth === 0) {
			matches.push(params.substr(matchFrom));
		}
	}
	return matches;
}

function pushInclude(state, imports, className) {
	if (className.indexOf('<') >= 0) {
		if (!resolveType(state, className)) {
			return;
		}
	}
