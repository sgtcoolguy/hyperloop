...
var fs = require('fs'),
	...
	log = require('../../log'),
	util = require('../../util'),
	primitiveTypeRegex = /^(u)?\s*(string|short|float|double|long|bool|int|char|unichar|_Bool)(32|64)?(?!acter)/i,
...
const
	...
	BLACKLIST_METHODS = {
		...
		'StorageItemThumbnail': [ 'Close' ],
		'StreamedFileDataRequest': [ 'Close' ],
...
function resolveObject(state, result) {
	...
	if (simpleType && simpleType.indexOf('IAsync') >= 0) {
...
function resolveType(state, type) {
	...
	if (is_primitive && type.toLowerCase() === 'string') {
		realtype = 'String^';
	}
	...
	if (!is_void && !is_function_pointer && !typeObject && !is_primitive && !is_block && !is_void_pointer) {
		...
		else if (type.toLowerCase() === 'guid') {
			return {
				metatype: 'struct',
				realtype: type.toLowerCase(),
				type: name.toLowerCase(),
				name: name.toLowerCase(),
				simpleType: simpleType.toLowerCase(),
				extendsClass: false,
				object: {},
				is_interface: true,
				is_object: true,
				is_enum: true,
				is_struct: true,
				instanceName: name.replace('const ', '').trim().toLowerCase(),
				fullInstanceName: '',
				mangledName: mangled.toLowerCase()   //mangle the name so we can build a consistent and safe function name from it
			};
		} else {
	...
	if (is_generic) {
		...
		fullInstanceName = fullInstanceName.replace(/\./g, '::');
	}
...
function makeExceptionHandler(code, returnValue) {
	...
	code.unshift('try\n{');
	code.push('}');
	code.push('catch (JSValueRef exc)');
	code.push('{');
	code.push('\t*exception = exc;');
	returnValue && code.push('\treturn ' + returnValue + ';');
	code.push('}');
	code.push('catch(Exception ^ex)');
	code.push('{');
	code.push('\tHyperloopRaiseNativeToJSException(ctx, exception, ex, __FILE__, __FUNCTION__, __LINE__ - ' + lineCount + ');');
	returnValue && code.push('\treturn ' + returnValue + ';');
	code.push('}');
}
...
function makeMethod(state, typeobj, methodName, resultName, instanceName, argumentsName, argumentsLengthName, returnFilterFunc) {
	...
	_.compact(methods).forEach(function(method) {
		...
		if (typeobj.simpleType === 'Window') {
			...
			target = convertToVarName(typeobj.mangledName);
		}
